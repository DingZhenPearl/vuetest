...
// 导入第一章数据
async function importChapter1() {
  try {
    const chapterData = {
      teacher_email: teacherEmail,
      chapter_id: chaptersData[0].id,
      chapter_number: chaptersData[0].number,
      chapter_title: chaptersData[0].title,
      chapter_difficulty: chaptersData[0].difficulty,
      chapter_description: chaptersData[0].description,
      sections: chaptersData[0].sections
    };
    
    console.log(`正在导入章节: ${chaptersData[0].title}`);
    ...
    if (result.success) {
      console.log(`章节 "${chaptersData[0].title}" 导入成功`);
    } else {
      console.error(`章节 "${chaptersData[0].title}" 导入失败: ${result.message}`);
    }
  } catch (error) {
