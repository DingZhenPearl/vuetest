    ...
    for chapter in CHAPTERS_DATA:
        ...
        try:
            response = requests.post(
                "http://localhost:3000/api/teaching-content/chapters",
                json=chapter_data
            )
            ...
            if result.get("success"):
                print(f"章节 '{chapter['title']}' 导入成功")
            else:
                print(f"章节 '{chapter['title']}' 导入失败: {result.get('message')}")
        except Exception as e:
            print(f"导入章节失败: {str(e)}")
